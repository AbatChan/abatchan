...
// Add pricing to the existing navigation without duplicating it.
...
.intro video{position:absolute!important;left:50%!important;top:50%!important;width:min(30vw,460px)!important;height:auto!important;max-width:460px!important;max-height:30vh!important;transform:translate(-50%,-50%)!important;object-fit:contain!important;background:#0c0c0c!important;display:block!important}
.skip{z-index:2}
footer{padding:22px 0 28px!important;border-top:1px solid var(--line)!important}
.compact-footer{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:24px}
.footer-mark{display:inline-flex;align-items:center}.footer-mark img{display:block;width:118px;height:auto}
.social-rail{display:flex;justify-content:center;gap:7px;flex-wrap:wrap}
.social-rail a,.social-rail button{width:34px;height:34px;border-radius:10px;border:1px solid var(--line);background:rgba(245,245,243,.025);color:var(--muted);display:grid;place-items:center;font:600 10px/1 Geist,Inter,sans-serif;letter-spacing:.03em;transition:transform .22s,background .22s,color .22s,border-color .22s}
.social-rail a:hover{transform:translateY(-3px);background:rgba(99,102,241,.14);border-color:rgba(99,102,241,.6);color:var(--paper)}
.social-rail button{cursor:not-allowed;opacity:.48}
.footer-meta{display:flex;align-items:center;gap:16px;color:var(--muted);font-size:12px;white-space:nowrap}.footer-meta a:hover{color:var(--paper)}
...
@media(max-width:900px){.site-header .brand img{width:118px!important}.intro video{width:min(52vw,320px)!important;max-height:34vh!important}.compact-footer{grid-template-columns:1fr;text-align:center;justify-items:center;gap:16px}.footer-meta{white-space:normal}.mobile-tabs a{font-size:9px}}
