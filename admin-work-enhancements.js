// Compact homepage summary plus ordered gallery controls.
(function adminWorkEnhancements(){
  ...
  style.textContent=`
    ...
    .adm-feature-extra[hidden]{display:none}.adm-feature-extra .adm-mini-note{margin:0;color:var(--muted);font-size:12px;line-height:1.55}
    ...
    .adm-media-body{display:grid;gap:12px;padding-top:14px}.adm-gallery-input{display:none}.adm-gallery-drop{display:grid;place-items:center;min-height:96px;padding:16px;border:1px dashed var(--edge);border-radius:14px;color:var(--muted);font-size:13px;text-align:center;cursor:pointer;transition:.22s}.adm-gallery-drop:hover,.adm-gallery-drop.over{border-color:var(--signal);background:rgba(99,102,241,.07);color:var(--paper)}
  ...
  const chooseFiles=files=>{
    const valid=[...files].filter(file=>file.type.startsWith('image/')&&file.size<=5*1024*1024);const room=Math.max(0,6-entries.length);
    valid.slice(0,room).forEach(file=>entries.push({file,objectUrl:URL.createObjectURL(file)}));renderGallery();markDirty();setStatus(valid.length>room?'Only six extra images are kept. Drag them to set display order.':'Drag images or use arrows to set display order.',valid.length>room);
  };
  ...
  const wait=()=>{
    ...
    const block=document.createElement('section');block.className='adm-feature-extra';block.id='workEnhancements';block.hidden=!featured.checked;block.innerHTML=`<div><label for="f-home-summary">homepage summary</label><textarea id="f-home-summary" maxlength="180" placeholder="Short version used only on the homepage."></textarea><p class="adm-mini-note">Shown only when this project is Featured. Leave empty to reuse the main summary.</p></div><details class="adm-media-details"><summary>Gallery images <span style="color:var(--muted);font-size:12px">optional · cover image stays first</span></summary><div class="adm-media-body"><div class="adm-gallery-drop" id="galleryDrop" tabindex="0" role="button">Drop images here, or click to choose<br><small>Up to 6 extras · drag to reorder</small></div><input class="adm-gallery-input" id="galleryInput" type="file" accept="image/*" multiple><div class="adm-gallery-grid" id="galleryGrid"></div></div></details><div class="adm-enhance-status" id="enhanceStatus"></div>`;actions.before(block);
    featured.addEventListener('change',()=>{block.hidden=!featured.checked;markDirty()});q('#f-home-summary').addEventListener('input',markDirty);const drop=q('#galleryDrop'),input=q('#galleryInput');drop.addEventListener('click',()=>input.click());drop.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();input.click()}});input.addEventListener('change',()=>chooseFiles(input.files));['dragenter','dragover'].forEach(type=>drop.addEventListener(type,e=>{e.preventDefault();drop.classList.add('over')}));['dragleave','drop'].forEach(type=>drop.addEventListener(type,e=>{e.preventDefault();drop.classList.remove('over')}));drop.addEventListener('drop',e=>chooseFiles(e.dataTransfer.files));
    editor.addEventListener('submit',()=>{pendingSave=true},{capture:true});const view=q('#view-editor');new MutationObserver(()=>{if(!view.classList.contains('adm-hide')){block.hidden=!featured.checked;setTimeout(()=>loadProject(true),0);return}if(pendingSave){pendingSave=false;saveExtras()}}).observe(view,{attributes:true,attributeFilter:['class']});q('#f-slug').addEventListener('change',()=>loadProject(true));return true;
  };
