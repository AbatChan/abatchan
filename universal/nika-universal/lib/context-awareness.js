...
export function isCurrentLocationQuestion(value) {
  const message = compact(value, 1000).toLowerCase().replace(/[’]/g, "'").replace(/[\s?.!]+$/, '');
  if (!message) return false;
  // Keep the deterministic shortcut for a pure location question only. A
  // compound request must reach the model so its second half is not dropped.
  return /^(?:where\s+(?:am\s+i|are\s+we)(?:\s+(?:now|currently|right now))?|(?:do\s+you\s+know\s+)?(?:what|which)\s+page\s+(?:(?:am\s+i|are\s+we)\s+on|is\s+this|we(?:'re|\s+are)\s+on)|(?:what|which)\s+section\s+(?:(?:am\s+i|are\s+we)\s+(?:in|on|viewing)|is\s+this)|what\s+(?:am\s+i|are\s+we)\s+looking\s+at|what(?:'s|\s+is)\s+(?:currently\s+)?in\s+view|what(?:'s|\s+is)\s+(?:currently\s+)?on\s+(?:the\s+)?screen(?:\s+(?:rn|now|right now|currently))?)$/.test(message);
}
