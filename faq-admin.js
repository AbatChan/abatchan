...
(function faqAdmin(){
  ...
  let items=[],loaded=false,dirty=false,expanded=null,dragged=null,target=null,revision=0,saveQueue=Promise.resolve(),query='',statusFilter='all';
  ...
  q('#view-faqs')?.remove();const section=document.createElement('section');section.id='view-faqs';section.className='adm-hide faq-admin-shell';section.innerHTML=`<div class="adm-head"><h2>FAQs</h2><button class="btn primary sm" id="saveFaqs" type="button">save <span class="arrow">↗</span></button></div><p class="adm-sub">Pricing-page questions. Drag on desktop, or use the up and down buttons on any screen.</p><div class="adm-list-tools"><label class="adm-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6.5"/><path d="m16 16 4.5 4.5"/></svg><input type="search" id="faqSearch" placeholder="Search FAQs" aria-label="Search FAQs"></label><div class="adm-seg" role="group" aria-label="Filter by status"><button type="button" data-status="all" aria-pressed="true">all</button><button type="button" data-status="published" aria-pressed="false">published</button><button type="button" data-status="draft" aria-pressed="false">drafts</button></div></div><div class="faq-admin-toolbar"><span class="faq-admin-count" id="faqAdminCount">0 FAQs</span><button class="btn sm" id="addFaq" type="button">add FAQ <span class="arrow">↗</span></button></div><div class="faq-admin-status" id="faqAdminStatus"></div><div class="faq-admin-list" id="faqAdminList"></div>`;main.append(section);
  ...
  const matches=item=>{
    ...
    return [item.question,item.answer].filter(Boolean).join(' ').toLowerCase().includes(query);
  };
  const render=()=>{
    ...
    count.textContent=filtering?`${view.length} of ${items.length} shown`:`${items.length} FAQ${items.length===1?'':'s'}`;
  ...
  q('#faqSearch').addEventListener('input',e=>{query=e.target.value.trim().toLowerCase();render()});
