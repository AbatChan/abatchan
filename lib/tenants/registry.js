...
export async function resolveTenant(siteKey) {
  // This deployment's own tenant ships with the code, so a stale private row
  // cannot contradict the public site after a release. Buyer tenants remain in
  // their private store and keep their independent settings and content.
  const record = siteKey === PRIMARY_SITE_KEY ? abatchan : await tenantStore.get(siteKey);
