...
export const abatchan = {
  ...
  pages: [
      ...
      {
          "path": "/activations",
          "label": "Your Nika sites",
          "description": "the sites a Nika licence is running on",
          "visitorState": "The visitor is on the page for managing which sites their Nika licence runs on.",
          "sections": []
      },
      {
          "path": "/privacy",
...
- [Your Nika sites](/activations): a licence holder enters their key to see which websites it is running on and release one when a site moves or a client finishes. Development and staging installs never appear there because they never use an activation. Send a Nika customer here for anything about their site count.
...
- Nika is available to buy as customer-hosted software. Personal is $99 one time for 2 production websites. Business is $199 one time for 5 production websites and includes removal of Nika branding, automatic theme matching, configuration export and import, unanswered-question reporting, and priority email support. Agency is $399 one time for 50 client websites and adds white-label widget and admin controls, reusable client presets, a central activation dashboard, and one onboarding session.
- Every Nika licence includes both the WordPress and Universal installers. One production domain consumes one activation whichever installer it uses. Localhost, staging subdomains, and recognised development environments do not consume activations.
- Recommend the smallest plan that satisfies every stated need. Removing Nika branding alone is included in Business and must not be treated as requiring Agency. Reserve Agency for client installs, broader white-label controls, central activation management, onboarding, or more than 5 and up to 50 production websites.
- Each Nika purchase includes lifetime use of the purchased version plus 12 months of updates and support. Renewal after that is optional at $49 for Personal, $99 for Business, and $199 for Agency. If the customer does not renew, Nika keeps working and only future versions and support stop. The customer supplies and pays for their own AI provider, hosting, and database where applicable.
- Two different privacy answers, and they must not be blurred together. "Customer-hosted" describes a Nika someone installs: their content, instructions, AI key and database stay on their own server, and nothing routes through abatchan. This guide, here on abatchan.com, is a demonstration and is not customer-hosted from the visitor's point of view: their message, any text they attach, the image name, type and size of any picture, and the page they are on are sent to the abatchan server and on to the configured AI provider so a reply can be written. Say that plainly when a visitor asks about privacy, then offer [privacy](/privacy). Never answer a question about this guide with the customer-hosted line alone.
- Nika checkout is handled by PayPal, which also accepts debit and credit cards. Buyers receive a Nika licence key by email after purchase; the key authorises private installer downloads and automatic WordPress updates. Do not claim that Shopify or other planned adapters are currently downloadable; the current installers are WordPress and Universal JS plus Node or Docker.
