 ...
 *
 * Only descriptive notes belong here — what a control is, or an illustration of
 * a value. Anything that changes what the person types (a format, a fallback
 * rule, a state) stays visible under the input, because a hidden note cannot
			...
			<main class="nika-main">
				...
				<section class="nika-card" id="nika-identity">
					...
					<div class="nika-grid nika-grid--2">
						...
						<?php endforeach; ?>
						<label class="nika-field"><span><?php esc_html_e( 'Panel colour', 'nika-site-guide' ); ?><?php echo nika_help( __( 'The panel surface behind the conversation.', 'nika-site-guide' ) ); ?></span><span class="nika-colour"><input name="<?php echo esc_attr( NIKA_OPTION ); ?>[panel_colour]" type="color" value="<?php echo esc_attr( $s['panel_colour'] ); ?>"><code><?php echo esc_html( strtoupper( $s['panel_colour'] ) ); ?></code></span></label>
						<label class="nika-field"><span><?php esc_html_e( 'Panel opacity', 'nika-site-guide' ); ?><?php echo nika_help( __( 'Lower lets the page show through the blur behind the panel.', 'nika-site-guide' ) ); ?></span><span class="nika-range"><input name="<?php echo esc_attr( NIKA_OPTION ); ?>[panel_opacity]" type="range" min="20" max="100" step="1" value="<?php echo esc_attr( $s['panel_opacity'] ); ?>"><output><?php echo esc_html( $s['panel_opacity'] ); ?>%</output></span></label>
						<label class="nika-field"><span><?php esc_html_e( 'Bubble gradient start', 'nika-site-guide' ); ?><?php echo nika_help( __( 'The accent colour sits between these two.', 'nika-site-guide' ) ); ?></span><span class="nika-colour"><input name="<?php echo esc_attr( NIKA_OPTION ); ?>[gradient_from]" type="color" value="<?php echo esc_attr( $s['gradient_from'] ); ?>"><code><?php echo esc_html( strtoupper( $s['gradient_from'] ) ); ?></code></span></label>
				...
				<section class="nika-card" id="nika-guardrails">
					...
					<div class="nika-grid nika-grid--4"><label class="nika-field"><span><?php esc_html_e( 'Per visitor / hour', 'nika-site-guide' ); ?><?php echo nika_help( __( 'Example: 20 lets one visitor ask up to 20 questions before the hourly reset.', 'nika-site-guide' ) ); ?></span><input id="nika-limit" name="<?php echo esc_attr( NIKA_OPTION ); ?>[hourly_limit]" type="number" min="1" max="1000" value="<?php echo esc_attr( $s['hourly_limit'] ); ?>"></label><label class="nika-field"><span><?php esc_html_e( 'Whole site / day', 'nika-site-guide' ); ?><?php echo nika_help( __( 'Example: 500 a day is at most 15,000 requests in a 30-day month.', 'nika-site-guide' ) ); ?></span><input name="<?php echo esc_attr( NIKA_OPTION ); ?>[daily_limit]" type="number" min="1" max="100000" value="<?php echo esc_attr( $s['daily_limit'] ); ?>"></label><label class="nika-field"><span><?php esc_html_e( 'Visible characters', 'nika-site-guide' ); ?><?php echo nika_help( __( 'Example: 12,000 characters is roughly 2,000 words from the current screen.', 'nika-site-guide' ) ); ?></span><input name="<?php echo esc_attr( NIKA_OPTION ); ?>[context_characters]" type="number" min="1000" max="20000" step="1000" value="<?php echo esc_attr( $s['context_characters'] ); ?>"></label><label class="nika-field"><span><?php esc_html_e( 'Conversation turns', 'nika-site-guide' ); ?><?php echo nika_help( __( 'Example: 10 keeps about 10 recent question-and-answer exchanges.', 'nika-site-guide' ) ); ?></span><input name="<?php echo esc_attr( NIKA_OPTION ); ?>[history_turns]" type="number" min="1" max="20" value="<?php echo esc_attr( $s['history_turns'] ); ?>"></label></div>
					<div class="nika-boundary-grid">
						<label class="nika-field"><span><?php esc_html_e( 'Excluded paths', 'nika-site-guide' ); ?><?php echo nika_help( __( 'Nika will not load, index, or navigate to these paths.', 'nika-site-guide' ) ); ?></span><textarea rows="5" id="nika-excluded" name="<?php echo esc_attr( NIKA_OPTION ); ?>[excluded_paths]" placeholder="/privacy&#10;/account"><?php echo esc_textarea( $s['excluded_paths'] ); ?></textarea><small><?php esc_html_e( 'One path per line.', 'nika-site-guide' ); ?></small></label>
						<div class="nika-field nika-exemptions"><span class="nika-field__head"><span><?php esc_html_e( 'Connections not counted', 'nika-site-guide' ); ?><?php echo nika_help( __( 'Testing and demo IPs skip visitor limits. Addresses in NIKA_EXEMPT_IPS always stay exempt.', 'nika-site-guide' ) ); ?></span></span><textarea rows="5" id="nika-exempt-ips" name="<?php echo esc_attr( NIKA_OPTION ); ?>[exempt_ips]" placeholder="203.0.113.24"><?php echo esc_textarea( $s['exempt_ips'] ); ?></textarea><span class="nika-field__actions"><button type="button" class="nika-generate nika-tip" id="nika-check-ip" data-tip="<?php esc_attr_e( "Check this browser's public IP before adding it", 'nika-site-guide' ); ?>"><svg viewBox="0 0 20 20" width="13" height="13" aria-hidden="true" focusable="false"><circle cx="10" cy="10" r="7" fill="none" stroke="currentColor" stroke-width="1.6"></circle><path d="M3 10h14M10 3a11 11 0 0 1 0 14A11 11 0 0 1 10 3Z" fill="none" stroke="currentColor" stroke-width="1.6"></path></svg><span><?php esc_html_e( 'My IP', 'nika-site-guide' ); ?></span></button><button type="button" class="nika-generate" id="nika-add-ip" hidden><span><?php esc_html_e( 'Add to list', 'nika-site-guide' ); ?></span></button></span><small id="nika-ip-status" role="status" aria-live="polite"></small></div>
					</div>
				</section>
...
function nika_system_prompt( $s, $pages, $page, $answer_depth = 'concise', $mode = 'json' ) {
	...
	return "You are {$s['name']}, the read-only website guide for " . home_url() . ".\n"
		...
		. "Every visitor message is a live request, including one that repeats or refines an earlier one. Having already taken the visitor somewhere is never a reason to decline, defer, or reply that something was handled before. If they ask again, ask to see a specific part, or say the last answer missed what they meant, do the work now. Resolve what a short follow-up refers to from the conversation rather than asking: highlight it, show me where they said that, and where exactly all point at whatever was just quoted, named or explained, most often in your own previous answer. Take that exact wording as the label and set section_requested true so the browser can reveal it on the page.\n"
...
function nika_direct_highlight_action( $message, $page, $pages ) {
	...
	foreach ( $matches as $match ) {
		...
		$contained = false;
		foreach ( $ordered as $kept ) {
			$match_end = (int) $match['position'] + (int) $match['length'];
			$kept_end = (int) $kept['position'] + (int) $kept['length'];
			if ( (int) $match['position'] >= (int) $kept['position'] && $match_end <= $kept_end ) {
				$contained = true;
				break;
			}
		}
