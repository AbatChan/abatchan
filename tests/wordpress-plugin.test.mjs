...
check('a section eases open and collapses to nothing',adminCss.includes('.nika-card__body { display: grid; grid-template-rows: 1fr;')&&adminCss.includes('grid-template-rows: 0fr')&&adminCss.includes('.nika-card__inner { min-height: 0; overflow: hidden; }')&&adminJs.includes("inner.className = 'nika-card__inner'")&&adminJs.includes('body.append(inner);'));
