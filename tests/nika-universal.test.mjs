...
const checks = [
  ...
  ['presets are a stored export, applied through the import reader', server.includes('document: exportDocument()') && server.includes('return importDocument(presets[id].document);')],
  ['and live in their own file, not in the config they set', server.includes("const PRESETS_PATH = join(DATA_DIR, 'presets.json');") && server.includes('carrying somebody else')],
  ['the same name replaces rather than duplicates', server.includes("String(preset?.name || '').toLowerCase() === name.toLowerCase()")],
  ['the list never carries the stored settings', server.includes('function presetList()') && !/function presetList\(\)[\s\S]{0,300}document:/.test(server)],
  ['every preset action checks the package', server.includes("if (!licence.can('client_presets')) return send(res, 403,")],
  ['applying asks before it replaces anything', adminJs.includes("apply.textContent='Apply, replacing these settings?'")],
  ['custom CSS cannot hide the credit without the capability', server.includes('function cssWithoutBrandHiding(') && server.includes("if (!css || licence.can('unbranded')) return css;") && server.includes('customCss: cssWithoutBrandHiding(')],
  ['and only the hiding declarations are dropped', server.includes("if (!/\\.assist-brand\\b/i.test(selector)) return rule;") && server.includes('Colour, spacing and font survive')],
